...
local function on_attach(client, bufnr)
  vim.api.nvim_buf_create_user_command(bufnr, 'CljLsp', function(info)
    if #info.fargs == 0 then
      vim.ui.select(cmds, {
        format_item = function(item) return ('%s | %s'):format(item[3], item[1]) end,
      }, function(item, _idx)
  ...
  vim.keymap.set('n', '<LocalLeader><LocalLeader>', '<Cmd>CljLsp<CR>', {
    buffer = bufnr,
    desc = 'Execute CljLsp command',
  })
end
...
return {
  ...
  handlers = {
    ...
    ['clojure/textDocument/testTree'] = function(_err, result, _ctx) vim.print(result) end,
    ['clojure/workspace/projectTree/nodes'] = function(_err, result, _ctx) vim.print(result) end,
  },
  on_attach = on_attach,
}
